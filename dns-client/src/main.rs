...
async fn main() {
    ...
    match &cli.command {
        Some(Commands::SetDns {
            ...
            if let Err(e) = commands::handle_set_dns_records(
                domain,
                enclave_ip,
                contract_address,
                wallet_private_key,
        ...
        Some(Commands::SetWhois {
            ...
            if let Err(e) = commands::handle_set_whois_delegation(
                domain,
                delegate_wallet_address,
                contract_address,
                wallet_private_key,
        ...
        Some(Commands::TransferDomain {
            domain,
            new_owner_wallet_address,
            contract_address,
            wallet_private_key,
        }) => {
            if let Err(e) = commands::handle_transfer_domain(
                domain,
                new_owner_wallet_address,
                contract_address,
                wallet_private_key,
        ...
        Some(Commands::SetKms {
            domain,
            kms_contract_address,
            contract_address,
            wallet_private_key,
        }) => {
            if let Err(e) = commands::handle_set_kms(
                domain,
                kms_contract_address,
                contract_address,
                wallet_private_key,
        ...
        Some(Commands::GrantRole {
            ...
            if let Err(e) =
                commands::handle_grant_role(role, account, contract_address, wallet_private_key)
                    .await
            {
                eprintln!("Error granting role: {}", e);
            }
        }
        Some(Commands::RevokeRole {
            role,
            account,
            contract_address,
            wallet_private_key,
            ...
            if let Err(e) =
                commands::handle_revoke_role(role, account, contract_address, wallet_private_key)
                    .await
            {
                eprintln!("Error revoking role: {}", e);
            }
        }
        Some(Commands::GetDomainOwnerRole {
            domain_id,
            contract_address,
        }) => {
            if let Err(e) =
                commands::handle_get_domain_owner_role(domain_id, contract_address).await
            {
                eprintln!("Error getting domain owner role: {}", e);
            }
        }
        Some(Commands::GetDomainManagerRole {
            domain_id,
            contract_address,
        }) => {
            if let Err(e) =
                commands::handle_get_domain_manager_role(domain_id, contract_address).await
            {
        ...
        Some(Commands::RetrieveDomain {
            ...
            if let Err(e) =
                commands::handle_retrieve_domain(domain, to, contract_address, wallet_private_key)
                    .await
            {
